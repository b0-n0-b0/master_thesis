...
def main():
    ...
    # NOTE: array containing found constraints per function
    found_constraints = {}
    ...
    for combo in generate_ordered_valid_combinations(rule_matches, is_valid_rule_match_sequence, key_order):
        ...
        for rule_match in rule_match_list:
            # print(f"The tool found {len(rule_match)} matches")
            ...
            for c in constraints:
                print(c, flush=True)
            found_constraints.setdefault(current_fidx, []).append(constraints)
    for fidx, constraints in found_constraints.items():
        ...
        # print(f"the subtree for function {fidx} has {len(edges)} edges", flush=True)
        for edge in edges:
